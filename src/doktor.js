...
export default () => {
    ...
    const submitForm = () => {
        ...
        axios.post('/api/add-doktor', { ad: ad })
            .then(function (response) {
                if (response.data.status === "success") {
                    fetchData()
                }
    ...
    return (
        <div>
            <Button onClick={submitForm}> Doktor Ekle</Button>
            <Form>
                <Form.Group>
                    <Form.Label>doktor Adı</Form.Label>
                    <Form.Control as="textarea" onChange={adOnChange} placeholder="doktor Adı" />
                </Form.Group>
            </Form>
            <Table>
